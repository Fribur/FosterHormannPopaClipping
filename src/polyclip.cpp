  ...
  for (polygon& P : PP) 
    for (vertex* I : P.vertices(INTERSECTION)) {
        ...
        while ((P_m->p - I->p) * (P_m->p - I->p) < EPSILON)
            P_m = P_m->prev;
        while ((P_p->p - I->p) * (P_p->p - I->p) < EPSILON)
            P_p = P_p->next;
        while ((Q_m->p - I->p) * (Q_m->p - I->p) < EPSILON)
            Q_m = Q_m->prev;
        while ((Q_p->p - I->p) * (Q_p->p - I->p) < EPSILON)
            Q_p = Q_p->next;
...
void cleanUpResult() {
  ...
  for (polygon& R : RR) {
    ...
    if (R.root != NULL) {
        vertexIterator IteratorAll = R.vertices(ALL);
        for (auto it = IteratorAll.begin(); it != IteratorAll.end(); ++it)
        {
            vertex* V = *it;
            if (fabs(A(V->prev->p, V->p, V->next->p)) < EPSILON) {
                auto next_it = it; //backup next position in the list
                ++next_it;
                R.removeVertex(V);//delete call inside this function invalidates iterator
                IteratorAll = R.vertices(ALL); //get new iterator on shortended list
                it = next_it; //restore next position in list
                count++;
            }
        }
    }
